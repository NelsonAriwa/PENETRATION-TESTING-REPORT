...
Operating systems used for reconnaissance activities
...
Fingerprint the web technologies (server, CMS, plugins, IP).
...
Read the HTTP response headers of the website, to see the server banner, status, cookies and redirects.

wafw00f

Detect whether a Web Application Firewall protects the target site.

dnsrecon

Enumerate all DNS records: Name servers, Mail Servers, Sender Policy Framework, Text Record, and Service Record. (NS, MX, SPF, TXT, SRV).
...
Local IP and MAC address identification
